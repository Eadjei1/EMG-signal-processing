function [scm_EMGs,scm_emg_threshold,SCM_Time_output,Time_output]=manual_onsetCheck_CCC(headerFile,filename,outputData,outputData_rect,marker_index, ...
...
window_size = round(0.02 * fs); % 10 ms moving window
baseline_duration = round(0.5 * fs); % First 500 ms for baseline
...
while channel <= length(emg_muscle)+1
    ...
    for marker = marker_index
                ...
                %new onset detection code 03/23/25-----
                %Compute baseline mean and standard deviation
%                 emg_rectified = (signal_rect(emg_interval));
%                 emg_smoothed = movmean(emg_rectified, window_size);
% 
%                 baseline_mean = mean(emg_smoothed(onset_duration:onset_duration+baseline_duration));
%                 baseline_std = std(emg_smoothed(onset_duration:onset_duration+baseline_duration));
% 
%                 low_threshold = baseline_mean + (16 * baseline_std);
%                 high_threshold = baseline_mean + (18 * baseline_std);
% 
%                 above_high = find(emg_smoothed > high_threshold);%, 1, 'first');
% 
%                 below_low = find(emg_smoothed(onset_duration:above_high) < low_threshold, 1, 'last');
%                 
%                 if isempty(below_low)
%                     emg_loc = above_high(1);
%                 else
%                     emg_loc = below_low;
%                 end
...
%adding a clause trudy 07/26/24
                emg_loc_sec = (emg_loc - onset_duration)./fs;
                %forget all those that are negative or earlier than second
                %cue
                logic_index = emg_loc_sec > 0;
                emg_loc2 = emg_loc(logic_index);
%end of clause 07/26/24 

                onset = marker-onset_duration+emg_loc2(1);
                onset_sec = (emg_loc2(1)-onset_duration)/fs;
                offset = marker-onset_duration+emg_loc2(end);
                %offset = marker-onset_duration+above_high(end); %03/25/25
                offset_sec = (emg_loc2(end)-onset_duration)/fs;
                %offset_sec = (above_high(end)-onset_duration)/fs; %03/25/25
...
%                 hold on
                ...
                hold on
                plot(onset_sec, signal_rect(onset),'*g');
                plot(offset_sec, signal_rect(offset),'*k');
